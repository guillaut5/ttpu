...
class WazeRouteCalculator(object):
    ...
    def __init__(self,  log_lvl=logging.INFO,region='EU'):
    ...
    def address_to_coords(self, address):
        ...
        response=self.makeSecureRequest(self.WAZE_URL + get_cords, url_options)
      ...
      #  print response.json()
        if len(response.json()) == 0 :
            raise WRCError("Waze ne trouve pas l'adresse %s" % address)
        ...
        street = response_json['street']
        postalAdress= response_json['name']
        ...
        return {"lon": lon, "lat": lat, "bounds": bounds,
                ...
                u"city":city, u'number':number,u'street':street}
    ...
    def get_route(self, npaths=1, time_delta=0):
        ...
        response = self.makeSecureRequest(self.WAZE_URL + routing_req, url_options, headers= headers)
        response_json = None
        try :        
            response_json = response.json()
        except:
            print "Unexpected error:", sys.exc_info()[0]
            raise WRCError("Unexpected error:")
    ...
    def makeSecureRequest (self, url, url_options,headers=None):
        retry=3
        i = 0
        response = None
        while retry > 0 :
            try :
                if headers == None : 
                    ...
                    response = requests.get(url, params=url_options, headers = headers)
                retry = -1
            except Exception, e:
                retry = retry - 1
                i = i+1
